...
| Progress range validation | INTENTIONALLY DIFFERENT | Upstream `BackEvent` requires `progress in 0F..1F`. Rust `BackEvent::progressed(_with)` stores the value as given (`0.0..=1.0` by convention, documented on the type) and never panics in the constructor; platform adapters may clamp or reject out-of-range values at their boundary. |
...
| Apple application lifecycle | INTENTIONALLY DIFFERENT | macOS, UIKit and WatchKit adapters observe process-wide notifications with `objc2`; they do not model individual scenes. Explicit teardown is Rust RAII: callers drive terminal state via the exposed `registry()` (`registry().destroy()`) and observer unregistration happens on `Drop` (watchOS `Drop` also destroys its registry, since WatchKit has no termination notification). macOS synthetic notification delivery and observer removal were runtime-tested; UIKit and WatchKit were compile-tested only. |
